...
abstract class ApiRefTestBase extends WebDriverTestBase {
   ...
   */
  protected function uploadFile(string $filename): void {
    $this
      ->getSession()
      ->getPage()
   ...
   */
  protected function clickSubmit(): void {
   ...
   */
  protected function selectManualMode(): void {
    $this->selectMode('manual', ['id_or_name', self::TITLE_NAME]);
  }

  /**
   * Selects the 'upload' mode on the api_reference node form.
   */
  protected function selectUploadMode(): void {
    $this->selectMode('upload', ['id_or_name', self::FILEFIELD_NAME]);
  }
   ...
   *   successful.
   */
  protected function selectMode(string $mode, array $locator): void {
    $this
      ->getSession()
      ->getPage()
  ...
  /**
   * Clicks on the Add API Category button.
   */
  protected function clickAddApiCategory(): void {
    $this->click('input[name="field_api_category_test_add_more"]');
    $this->assertSession()->waitForElement('named', [
      'id_or_name',
      'input[name="field_api_category_test_add_more"]',
    ]);
  }
